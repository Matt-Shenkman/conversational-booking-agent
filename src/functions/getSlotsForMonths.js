...
module.exports = async function getSlotsForMonths(monthList = []) {
  ...
  if (!Array.isArray(monthList) || monthList.length === 0) {
    console.log("ℹ️ No months provided. Defaulting to current and next 2 months.");
    for (let i = 0; i < 3; i++) {
    ...
    for (const m of monthList) {
      ...
      if (inputMonth.isValid() && (inputMonth.isSame(now, 'month') || inputMonth.isAfter(now, 'month')) && diff <= 2) {
        console.log(`✅ Accepted month: ${m}`);
        validMonths.push(m);
      } else {
        console.warn(`⛔️ Rejected month: ${m} (diff: ${diff})`);
      }
    }
    ...
    if (validMonths.length === 0) {
      ...
      return {
        success: false,
        error: 'All provided months are invalid. Must be in YYYY-MM format and within 3-month window from today.',
      };
    }
  }
  ...
  try {
    ...
    const slotPromises = validMonths.map((month, idx) => {
      ...
      return getSuggestedSlots(month, pages[idx]);
    });
